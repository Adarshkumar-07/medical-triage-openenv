...
def run_evaluation(
    ...
    print(f"\n{'=' * 70}")
    print("  AI Medical Triage & Clinical Documentation Environment")
    print("  Inference & Evaluation Run")
    print(f"{'=' * 70}")
    ...
    print(
        "  LLM judge: "
        f"{'Enabled (ANTHROPIC_API_KEY set)' if api_key else 'Disabled'}"
    )
    print(f"{'=' * 70}\n")
...
for difficulty_str in difficulties:
    ...
    print(
        f"  ┌─ Difficulty: {
            difficulty_str.upper()} ({n_episodes_per_difficulty} episodes)")
    print(f"  │")
    ...
    for ep_idx in range(n_episodes_per_difficulty):
        ...
        print(
            f"  │  Ep {
                ep_idx + 1:02d}/{n_episodes_per_difficulty} | " f"ESI_GT={
                ep_result.esi_ground_truth} ESI_pred={
                ep_result.esi_predicted} | " f"Reward={
                    ep_result.total_reward:+.3f} | Grader={
                        ep_result.grader_score:.3f} | {status}")
    ...
    print(
        f"  └─ Mean steps: {
            report.mean_steps:.1f} | Mean elapsed: {
            report.mean_elapsed_seconds:.1f}s")
    print()
...
report_obj = EvaluationReport(
    env_id=TriageEnv.ENV_ID,
    env_version=TriageEnv.VERSION,
    agent="BaselineAgent",
    timestamp=time.time(),
    total_episodes=len(all_results),
    overall_pass_rate=sum(all_pass) /
    len(all_pass) if all_pass else 0.0,
    overall_mean_reward=sum(all_rewards) /
    len(all_rewards) if all_rewards else 0.0,
    overall_mean_grader_score=sum(all_grader) /
    len(all_grader) if all_grader else 0.0,
    difficulty_reports=difficulty_reports,
...
grade_result = GraderRegistry.from_string(
    grader_tier, config={
        "api_key": api_key}).grade(trajectory)
state = new_env.state()
...
return EpisodeResult(
    ...
    esi_predicted=esi_pred,
    steps_taken=trajectory.total_steps,
    elapsed_seconds=round(
        elapsed,
        3),
    total_reward=round(
        last_reward.total_reward,
        4) if last_reward else 0.0,
    scaled_score=round(
        last_reward.scaled_score,
        2) if last_reward else 0.0,
    triage_accuracy=round(
        last_reward.triage_accuracy_score,
        4) if last_reward else 0.0,
    documentation=round(
        last_reward.documentation_score,
        4) if last_reward else 0.0,
    diagnostic_appropriateness=round(
        last_reward.diagnostic_appropriateness_score,
        4) if last_reward else 0.0,
    treatment_safety=round(
        last_reward.treatment_safety_score,
        4) if last_reward else 0.0,
    time_efficiency=round(
        last_reward.time_efficiency_score,
        4) if last_reward else 0.0,
    penalty_flags=list(
        set(all_flags)),
    ...
    grader_score=round(
        ...
        4),
    grader_pass=grade_result.pass_fail,
    grader_feedback_summary=feedback_summary,
    terminated_by=state.get(
        "terminated_by",
        "unknown"),
)
...
def _compute_difficulty_report(difficulty_str, episodes) -> DifficultyReport:
    ...
    if n == 0:
        return DifficultyReport(
            ...
            mean_scaled_score=0.0,
            mean_grader_score=0.0,
            mean_triage_accuracy=0.0,
            mean_documentation=0.0,
            mean_diagnostic_appropriateness=0.0,
            mean_treatment_safety=0.0,
            mean_time_efficiency=0.0,
            critical_miss_rate=0.0,
            allergy_violation_rate=0.0,
            unsafe_discharge_rate=0.0,
            mean_steps=0.0,
...
def flag_rate(flag): 
    return sum(1 for ep in episodes if flag in ep.penalty_flags) / n
...
print()
col_w = 14
print(f"  {'Difficulty':<12}" +
      f"{'Pass Rate':>{col_w}}" +
      f"{'Mean Reward':>{col_w}}" +
      f"{'Grader Score':>{col_w}}" +
      f"{'Triage Acc':>{col_w}}" +
      f"{'Doc Score':>{col_w}}" +
      f"{'Safety':>{col_w}}")
print(f"  {'-' * 12}" + f"{'-' * col_w}" * 6)
for diff_str, dr in report.difficulty_reports.items():
print(f"  {diff_str.capitalize():<12}" +
      f"{dr.pass_rate:>{col_w}.1%}" +
      f"{dr.mean_total_reward:>{col_w}.4f}" +
      f"{dr.mean_grader_score:>{col_w}.4f}" +
      f"{dr.mean_triage_accuracy:>{col_w}.4f}" +
      f"{dr.mean_documentation:>{col_w}.4f}" +
      f"{dr.mean_treatment_safety:>{col_w}.4f}")
print(f"  {'-' * 12}" + f"{'-' * col_w}" * 6)
print(f"  {'OVERALL':<12}" +
      f"{report.overall_pass_rate:>{col_w}.1%}" +
      f"{report.overall_mean_reward:>{col_w}.4f}" +
      f"{report.overall_mean_grader_score:>{col_w}.4f}")
print(f"{'=' * 70}\n")
print("  Safety Metrics by Difficulty:")
for diff_str, dr in report.difficulty_reports.items():
print(
    f"    {
        diff_str.capitalize():<8} | Critical Miss: {
            dr.critical_miss_rate:.1%} | " f"Allergy Violations: {
                dr.allergy_violation_rate:.1%} | " f"Unsafe Discharge: {
                    dr.unsafe_discharge_rate:.1%}")
print(f"\n{'=' * 70}")
...
return {
    ...
    "difficulty_reports": difficulty_reports_out,
}
...
parser = argparse.ArgumentParser(
    description="Run AI Medical Triage evaluation.")
...
difficulties = [
    "easy",
    "medium",
    "hard"] if args.difficulty == "all" else [
        args.difficulty]
report = run_evaluation(
    n_episodes_per_difficulty=args.episodes,
