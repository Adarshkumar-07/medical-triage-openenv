...
for difficulty_str in difficulties:
    ...
    for ep_idx in range(n_episodes_per_difficulty):
        ...
        episode_results.append(ep_result); all_results.append(ep_result)
...
obs = new_env.reset(difficulty=difficulty, seed=seed); agent.reset()
done = False; last_reward = None; ep_start = time.monotonic()
while not done:
action = agent.act(obs); result = new_env.step(action)
last_reward = result.reward; done = result.done
...
n = len(episodes)
if n == 0:
return DifficultyReport(difficulty=difficulty_str, n_episodes=0, n_pass=0,
...
report = run_evaluation(n_episodes_per_difficulty=args.episodes, difficulties=difficulties,
